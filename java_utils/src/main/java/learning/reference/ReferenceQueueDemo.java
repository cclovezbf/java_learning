...
//        System.gc();
    ...
    private void test2() throws InterruptedException {
        ...
        for (int i = 0; i < 5; i++) {
            WeakReference<String> weakReference = new WeakReference<String>(String.valueOf(i), referenceQueue);
            System.out.println("创造了:key="+weakReference.get());
            map.put(weakReference, value);
        }
        System.gc();
        Thread.sleep(100);
        ...
        map.entrySet().stream().map(s->{
            WeakReference key = (WeakReference)s.getKey();
            return key.get()+"---"+s.getValue();
        }).forEach(System.out::println);
        int cnt = 0;
        while ((result=(WeakReference)referenceQueue.poll())!=null){
            System.out.println((cnt++) + "回收了:" + result+",key="+result.get());
        }
    }
